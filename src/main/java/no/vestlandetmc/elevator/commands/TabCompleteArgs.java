...
public class TabCompleteArgs implements TabCompleter {
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		if(args.length == 0) {
			...
			if("add".startsWith(args[0])) { commandList.add("add"); }
			if("remove".startsWith(args[0])) { commandList.add("remove"); }
			if("link".startsWith(args[0])) { commandList.add("link"); }
			if("unlink".startsWith(args[0])) { commandList.add("unlink"); }
			if("list".startsWith(args[0])) { commandList.add("list"); }
			if("help".startsWith(args[0])) { commandList.add("help"); }
